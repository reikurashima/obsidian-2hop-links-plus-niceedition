...
export default class TwohopLinksPlugin extends Plugin {
  ...
  private async openTagPage(property: string, key: string): Promise<void> {
    // For links-based groups, the property is already a page name
    // For tags, try to open a page with that tag name
    ...
    // If no file found, ask to create
    if (!confirm(`Create new file: ${pageName}?`)) {
      return;
    }
    return this.app.workspace.openLinkText(pageName, sourcePath);
  }
  ...
  async injectTwohopLinks(
    ...
    ReactDOM.render(
      <TwohopLinksRootView
        ...
        onTagClick={this.openTagPage.bind(this)}
